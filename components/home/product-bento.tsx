...
import { ArrowRight, Bot, MessageSquare, Brain, Search, Mic } from 'lucide-react';
    ...
    <section className="py-24 bg-background">
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
        <div className="max-w-4xl mx-auto text-center mb-16">
          <h2 className="text-3xl sm:text-4xl font-bold mb-4">{t('home.title')}</h2>
          ...
          >
            ...
            <div className="p-8 h-full flex flex-col">
              <div className="mb-4">
                 ...
                 <h3 className="text-2xl font-bold mb-2">{t('navigation.helpDesk')}</h3>
                 <p className="text-muted-foreground">{t('home.productBento.helpdeskDesc')}</p>
              </div>
              ...
              <div className="mt-auto relative w-full h-[180px] rounded-tl-2xl bg-card border-t border-l border-white/10 p-4 overflow-hidden translate-x-4 translate-y-4 group-hover:translate-x-2 group-hover:translate-y-2 transition-transform duration-500">
                 <div className="flex gap-2 items-center mb-4">
                    ...
                    <div className="bg-primary/20 px-3 py-2 rounded-xl text-xs text-white">{t('home.productBento.helpdeskUser')}</div>
                 </div>
                 <div className="flex gap-2 items-start ml-10">
                    <div className="bg-[#1a1a24] px-3 py-2 rounded-xl text-xs text-emerald-400 border border-emerald-500/30">
                      {t('home.productBento.helpdeskSystem')}
                    </div>
                 </div>
              </div>
          ...
          >
            ...
            <div className="p-6 h-full flex flex-col">
              <div className="flex items-center gap-3 mb-2">
                 ...
                 <h3 className="text-xl font-bold">{t('navigation.aiAgent')}</h3>
              </div>
              <p className="text-sm text-muted-foreground mb-4">{t('home.productBento.aiAgentDesc')}</p>
              ...
              <div className="mt-auto flex items-center justify-between text-xs text-muted-foreground bg-card p-3 rounded-lg border border-white/5">
                 <div className="flex items-center gap-2"><Search className="w-3 h-3 text-emerald-500" /> {t('home.productBento.aiAgentSearch')}</div>
              </div>
          ...
          >
            ...
            <div className="p-6 h-full flex flex-col">
              <div className="flex items-center gap-3 mb-2">
                 ...
                 <h3 className="text-xl font-bold">{t('navigation.asr')}</h3>
              </div>
              <p className="text-sm text-muted-foreground mb-4">{t('home.productBento.asrDesc')}</p>
              
              <div className="mt-auto flex items-end justify-center gap-1 h-12 opacity-50 group-hover:opacity-100 transition-opacity">
                 {[...Array(12)].map((_, i) => (
                    <div key={i} className="w-1.5 bg-blue-500 rounded-full animate-pulse" style={{ height: `${20 + (i % 3) * 25}%` }} />
                 ))}
              </div>
